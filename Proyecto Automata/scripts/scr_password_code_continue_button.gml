///scr_password_code_continue_button()

//Get inputs
var code_input = string_upper(button.password_code_handler.code_text_field.text);

//Check if all inputs filled
if(code_input != ""){
    //Check if code is correct
    if(code_input == TEMP_USER_DATA.autentication_code){
        //Go to change password room
        scr_room_goto(rm_change_password);
    }else{
        var window_title = "Error";
        var window_description = "El código de recuperación de contraseña ingresado es incorrecto.";
        var window_buttons_list = ds_list_create();
        var window_accept_button = scr_create_default_window_accept_button();
        ds_list_add(window_buttons_list,window_accept_button);
        var window = scr_create_window(window_title,window_description,window_buttons_list);
        
        button.password_code_handler.code_text_field.text = "";
    }
}else{
    var window_title = "Error";
    var window_description = "No se rellenaron todos los espacios necesarios.";
    var window_buttons_list = ds_list_create();
    var window_accept_button = scr_create_default_window_accept_button();
    ds_list_add(window_buttons_list,window_accept_button);
    var window = scr_create_window(window_title,window_description,window_buttons_list);
}

//Destroy dummy
instance_destroy();
